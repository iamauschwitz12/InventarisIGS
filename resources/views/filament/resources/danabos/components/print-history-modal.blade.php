            ...
            <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                @forelse($records as $record)
                    @php
                        ...
                        $kodeAwal = $record->danaBos->kode_inventaris . '-' . str_pad($start, 5, '0', STR_PAD_LEFT);
                        $kodeAkhir = $record->danaBos->kode_inventaris . '-' . str_pad($end, 5, '0', STR_PAD_LEFT);
                    @endphp
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->danaBos->no_invoice ?? '-' }}
                        </td>
                        <td class="px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->danaBos->nama_barang ?? '-' }}
                        </td>
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->danaBos->lantai->lantai ?? '-' }}
                        </td>
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->danaBos->ruang->ruang ?? '-' }}
                        </td>
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
                        ...
                        <td colspan="8" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400"
                            ...
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <span class="text-lg font-medium"
                                    style="display: block; font-size: 1.125rem; font-weight: 500;">
                                    Belum ada riwayat cetak
                                </span>
                                <span class="text-sm" style="display: block; font-size: 0.875rem;">
                                    Silakan cetak QR code terlebih dahulu.
                                </span>
                            </div>
                        </td>
                    </tr>
