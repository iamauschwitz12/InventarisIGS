            ...
            <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                @forelse($records as $record)
                    @php
                        ...
                        $pribadi = $record->pribadi;
                        $kodeAwal = $pribadi ? ($pribadi->kode_inventaris . '-' . str_pad($start, 5, '0', STR_PAD_LEFT)) : '-';
                        $kodeAkhir = $pribadi ? ($pribadi->kode_inventaris . '-' . str_pad($end, 5, '0', STR_PAD_LEFT)) : '-';
                    @endphp
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                        ...
                        <td class="px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->pribadi->nama_barang ?? '-' }}
                        </td>
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->pribadi->lantai->lantai ?? '-' }}
                        </td>
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
                            {{ $record->pribadi->ruang->ruang ?? '-' }}
                        </td>
                        <td class="whitespace-nowrap px-6 py-4" style="padding: 12px 16px;">
