...
class DanaBosForm
{
        ...
        return $schema
            ->components([
                ...
                Select::make('tipe_aset_id')
                    ...
                    ->reactive()
                    ->afterStateUpdated(function ($state, callable $set, callable $get) {
                        $set('tipe_aset_kategori_id', null);
                        self::updateKodeInventaris($get, $set);
                    })
                    ->createOptionForm([
    ...
    protected static function updateKodeInventaris(callable $get, callable $set)
    {
        ...
        $tipeAsetDanaBosId = $get('tipe_aset_dana_bos_id');

        $gedung = $gedungId ? \App\Models\Gedung::find($gedungId) : null;
        $tipeAset = $tipeAsetId ? \App\Models\TipeAset::find($tipeAsetId) : null;
        $kategori = $kategoriId ? \App\Models\TipeAsetKategori::find($kategoriId) : null;
        $tipeAsetDanaBos = $tipeAsetDanaBosId ? \App\Models\TipeAsetDanaBos::find($tipeAsetDanaBosId) : null;

        if ($gedung && $jenjang && $tipeAset) {
            // 1. Gedung: ambil 1 huruf dari kiri + tambahkan "B" (untuk BOS)
            $kode = strtoupper(substr($gedung->nama_gedung, 0, 1)) . 'B';
            ...
            if ($tipeAsetIndex !== false) {
                $kode .= '-' . chr(65 + $tipeAsetIndex);
            }

            // 4. Tipe Aset Dana BOS: huruf langsung setelah tipe aset (tanpa "-")
            if ($tipeAsetDanaBos) {
                $tipeAsetDanaBosLIst = \App\Models\TipeAsetDanaBos::orderBy('id')->pluck('id')->toArray();
                $tipeAsetDanaBosIndex = array_search($tipeAsetDanaBos->id, $tipeAsetDanaBosLIst);
                if ($tipeAsetDanaBosIndex !== false) {
                    $kode .= chr(65 + $tipeAsetDanaBosIndex);
                }
            }

            // 5. Kategori: tambahkan nomor urut kategori
            if ($kategori) {
                $kategoriList = \App\Models\TipeAsetKategori::where('tipe_aset_id', $tipeAset->id)
                    ->orderBy('id')
                    ->pluck('id')
                    ->toArray();
                $kategoriIndex = array_search($kategori->id, $kategoriList);
